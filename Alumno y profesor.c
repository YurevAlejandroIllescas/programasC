...
void Menu_Alumnos(){
	...
	fputs(rAlumno.eTelefono, fichero);
	fputs(rAlumno.rFecCumpleanios.eDia, fichero);
	fputs(rAlumno.rFecCumpleanios.eMes, fichero);
	fputs(rAlumno.rFecCumpleanios.eAnio, fichero);	
	fputs(rAlumno.rDireccion.cCalle, fichero);
	fputs(rAlumno.rDireccion.eNumExt, fichero);
	fputs(rAlumno.rDireccion.eNumInt, fichero);
	fputs(rAlumno.rDireccion.cColonia, fichero);
	fputs(rAlumno.rDireccion.cAlcaldia, fichero);
	fputs(rAlumno.rDireccion.eCodigoP, fichero);
	fputs(rAlumno.cCarrera, fichero);
	fputs(rAlumno.eSemestre, fichero);*/
	fclose(fichero);
...
int LeerIdAlumno (TaCadena cOperacion, int eTotal, int eUltimoID){
	...
	return -1;
		...
		do{
			...
			printf("\n\nIngrese el id del alumno a %s: ", cOperacion);
